class Solution {
    ...
    private int dfs(int i, int[] prices, int buy, int[][] dp,int fee){
        ...
        if(i == prices.length - 1) {
            ...
            if(buy == 0) {
                return dp[i][buy] = prices[i] -fee;  // Profit from selling stock at the last price
            }
        ...
        if(buy == 1) {
            ...
            ans += Math.max(
                -prices[i] + dfs(i + 1, prices, 0, dp,fee),  // Buy the stock and move to the next day
                0 + dfs(i + 1, prices, 1, dp,fee)            // Skip buying (stay in the same state)
            );
        } else {
            // If we are not allowed to buy (buy == 0), meaning we can sell the stock
            ans += Math.max(
                prices[i] -fee + dfs(i + 1, prices, 1, dp,fee),  // Sell the stock and move to the next day
                0 + dfs(i + 1, prices, 0, dp, fee)          // Skip selling (stay in the same state)
            );
        }
